...
| **📋 复制粘贴生产模式** | 你只想负责创意和审核，其余提示词、桥接、补救都交给 Skill | 可复制粘贴的关键帧提示词 + 视频提示词 + 桥接镜头 + 保险素材 + 修复提示词 | 图生图/图生视频 + 剪映 |
| **🎞️ 漫剧模式** | 叙事驱动的内容、旁白为主的项目、需要快速迭代的场景 | 面板映射表 + 批量图生 Prompt + 剪映装配指南 | 即梦 出图 + 剪映 装配 |

> **怎么选？** 想要连续运镜的电影感画面 → **电影模式**。想最大程度少思考、直接复制粘贴提示词 → **复制粘贴生产模式**。想要更快出片、旁白驱动的有声漫剧 → **漫剧模式**。
...
3. 🎬 **"镜头接不上"** — `Join Contract` 镜头连接合同会逐个检查相邻镜头的前后状态、风险等级、是否允许硬切、桥接镜头、声音桥和失败备用剪法。AI 视频天生没记忆——这个技能给它装上记忆
...
- **👤 高级角色视觉开发** — 角色三视图、面部特写、全身比例、局部细节、色卡和参考图线稿/色稿/反推提示词流程
...
| **镜头连接合同 Join Contract** | 每两个镜头都输出：前镜结束状态、后镜开始状态、状态差异、风险等级、是否允许硬切、桥接提示词、保险素材、声音桥、失败备用剪法 |
| **高风险转场自动补桥** | 换地点、抱人/救人、躺变坐/站、双人互动、道具状态变化等禁止直接硬切，自动生成桥接镜头或插入镜头 |
...
| 功能 | 说明 |
|---|---|
| **低输入问卷** | 只问类型、主角关系、第一集钩子、结尾悬念、目标平台，其余由 Skill 默认补齐 |
| **关键帧提示词包** | 每个镜头生成 `IMAGE_START`、`IMAGE_END`，高风险动作额外生成 `IMAGE_MID` |
| **视频提示词包** | 每个镜头生成 `VIDEO_MAIN`，只聚焦动作、运镜、时间推进，减少画面重复描述 |
| **桥接镜头包** | 高风险连接自动生成 `VIDEO_BRIDGE`，用于解决空间跳、姿态跳、动作因果断裂 |
| **保险素材包** | 自动生成手、眼睛、道具、门、灯、手机、工具、脚步、环境、反应等 `VIDEO_INSERT` |
| **失败修复包** | 每个高风险镜头预置 `VIDEO_REPAIR`，生成失败后可直接复制粘贴重试 |
...
├── 🔗 连续性方案 + Join Contract（状态追踪、风险等级、桥接镜头、匹配剪辑）
├── 🎥 视频提示词包（时间分段、内嵌对白/音效/配乐）
├── 📋 复制粘贴提示词包（IMAGE_START / IMAGE_END / VIDEO_MAIN / VIDEO_BRIDGE / VIDEO_INSERT / VIDEO_REPAIR）
├── 🖼️ 封面/缩略图设计
└── ✅ 制作清单（素材→粗剪→声音→迭代→合规）
```
...
| **先接头再出片** | 每个相邻镜头先做 Join Contract，高风险连接先补桥再生成主视频 |
