...
每条 `VIDEO_MAIN` 正文前必须先写 Shot State Contract，确认首帧、屏幕方位、主体/道具状态、镜头覆盖模式、动作状态变化和终帧。首帧不能被画成静帧时，不要进入提示词正文。

时间轴内必须同时写“镜头位置/运动 + 画面动作 + 声音”。尾部的“镜头语言”只做一致性总结，不新增时间轴里没有出现的角度、切镜或运动。

### 10.2 分镜与视频提示词
| 镜头 | 时长 | 风险 | 固定资产 | 风格质感 | 首帧状态 | 屏幕方位 | 镜头覆盖模式 | 动作状态变化 | 终帧/可剪点 | 前后镜关系 | 声音/对白处理 | 主提示词ID |
|---|---:|---|---|---|---|---|---|---|---|---|---|---|
...
0-1s：[镜头位置/运动：建立画面和动作起点，保留自然起步；画面动作：写清谁在何处、看向哪里、道具在哪里]
1-4s：[镜头位置/运动：沿 Shot State Contract 的镜头路径推进；画面动作：1-2 个动作 beat，每个 beat 都必须是可见动作]
4-5s：[镜头位置/运动：停在可剪辑的景别/角度；画面动作：动作落稳或留出反应，明确最终姿态/道具状态/视线方向，给剪辑留 0.5s 余量]
[对白：如有，放在对应时间点]
[音效：环境音、动作音，放在对应时间点]
[配乐：如有，放在对应时间点]
...
失败修复：如果首帧不清，补 Shot State Contract；如果动作过密，先删动作或拆镜；如果镜头乱切，改成连续单镜头或拆 `VIDEO_INSERT`；如果身份漂移，强化固定资产引用；必要时使用 `VIDEO_REPAIR_[镜头号]`。
```
...
0-1.5 秒：[镜头位置/运动：建立画面和动作起点；画面动作：主体做什么]
     [音效：环境音/效果音]
1.5-4 秒：[镜头位置/运动：沿 Shot State Contract 的镜头路径推进；画面动作：主要动作，最多 1-2 个动作 beat]
     [对白：角色台词内容]
4-5 秒：[镜头位置/运动：停在终帧要求的景别/角度；画面动作：动作落稳、反应或结尾可剪点]
     [对白：台词] [音效：效果音]
     [配乐：音乐描述]
