...
每条 `VIDEO_MAIN` 正文前必须先写 Shot State Contract，确认首帧、屏幕方位、主体/道具状态、表演因果、镜头覆盖模式、动作状态变化和终帧。首帧不能被画成静帧时，不要进入提示词正文。

时间轴内必须同时写“镜头位置/运动 + 画面动作 + 表演因果 + 声音”。尾部的“镜头语言”只做一致性总结，不新增时间轴里没有出现的角度、切镜或运动。

当角色表情或情绪重要时，使用表演公式：`真实情绪 = 发生原因 + 表情细节 + 肢体动作 + 说话语气`。不要只写“害怕/愤怒/感动”，要写清楚角色因为看到/听到/意识到什么而产生反应，脸、身体、声音分别怎么变化。

### 10.2 分镜与视频提示词
| 镜头 | 时长 | 风险 | 固定资产 | 风格质感 | 首帧状态 | 屏幕方位 | 表演因果 | 镜头覆盖模式 | 动作状态变化 | 终帧/可剪点 | 前后镜关系 | 声音/对白处理 | 主提示词ID |
|---|---:|---|---|---|---|---|---|---|---|---|---|---|---|
...
Shot State Contract：
- 参考角色：[哪个 @图片 锁定身份/表情/服装；哪个 @视频 只参考动作节奏]
- 首帧状态：[第 0 秒能画成静帧的具体画面：主体在画面哪里、姿态、看向哪里、手/脚/道具在哪里、环境是什么]
- 屏幕方位：[主体运动方向、镜头左/右/前/后关系、前景/中景/背景]
- 主体状态：[身体姿态、情绪、即时意图]
- 表演因果：[当情绪重要时必须写：发生原因 -> 表情细节 -> 肢体动作 -> 说话语气；例如“刹车松空和货车逼近 -> 瞳孔放大、嘴角绷紧 -> 肩膀僵住、身体压低 -> 急促低声说'糟了'”]
- 道具状态：[关键道具的位置、归属、可见状态、允许发生的变化]
- 镜头覆盖模式：[连续单镜头 / 单镜头焦点转移 / 计划切镜序列 / 蒙太奇；默认选一个，不混用]
- 镜头路径：[镜头从哪里开始、如何运动、在哪里结束；若是连续镜头，明确“全程同一镜头，不做无关切换”]
- 动作状态变化：[本段唯一状态变化，例如“坐着->站起”“握住钥匙->插入锁孔”“危险逼近->停在未碰撞前”]
- 终帧状态：[最后 0.5 秒的具体姿态、道具状态、视线方向、可剪辑点]
- 硬限制：[不要推进到下一剧情；安全关键限制要配合正向目标，例如“保持安全距离，停在碰撞前”]

0-1s：[镜头位置/运动：建立画面和动作起点，保留自然起步；画面动作：写清谁在何处、看向哪里、道具在哪里；表演因果：如有情绪，写触发原因和第一反应]
1-4s：[镜头位置/运动：沿 Shot State Contract 的镜头路径推进；画面动作：1-2 个动作 beat，每个 beat 都必须是可见动作；表演因果：写脸部细节、肢体动作、语气如何随事件变化]
4-5s：[镜头位置/运动：停在可剪辑的景别/角度；画面动作：动作落稳或留出反应，明确最终姿态/道具状态/视线方向；表演因果：停在具体表情/呼吸/眼神/台词语气上，给剪辑留 0.5s 余量]
[对白：如有，放在对应时间点]
[音效：环境音、动作音，放在对应时间点]
[配乐：如有，放在对应时间点]
镜头语言一致性检查：[总结时间轴中已经写明的镜头路径；必须匹配镜头覆盖模式；不要新增时间轴里没有出现的角度、切镜或运动；如果需要手部/眼睛/道具特写，优先拆成 `VIDEO_INSERT`，不要在连续单镜头里要求快速交替特写]
结尾可剪点：[角色停顿/转头/道具落稳/反应表情/环境声延续]
不变项：[身份、服装、发型、道具、场景、光线方向中最重要的 3-6 项]
禁止：不要换地点，不要新增人物，不要烧录字幕，不要改变核心服装/道具状态，不要跳到下一镜头剧情。
可用剪辑范围：预计取中间 [x-y] 秒；首尾 0.5 秒可裁。
生成策略：建议生成 [3-8] 条，保留动作最清楚、身份最稳、结尾可剪点最好的一条。
常见失败：首帧不清 / 身份漂移 / 手部错乱 / 动作没完成 / 自己切镜 / 终帧提前进入下一剧情。
失败修复：如果首帧不清，补 Shot State Contract；如果表情假或情绪跳变，补“发生原因 -> 表情细节 -> 肢体动作 -> 说话语气”；如果动作过密，先删动作或拆镜；如果镜头乱切，改成连续单镜头或拆 `VIDEO_INSERT`；如果身份漂移，强化固定资产引用；必要时使用 `VIDEO_REPAIR_[镜头号]`。
```
...
Shot State Contract：
- 首帧状态：[0 秒静帧画面，主体位置/姿态/视线/手部/道具/环境]
- 屏幕方位：[运动方向、左右关系、前中背景]
- 表演因果：[情绪触发原因 + 表情细节 + 肢体动作 + 说话语气；情绪不重要时写“本镜头无明显情绪变化”]
- 镜头覆盖模式：[连续单镜头 / 单镜头焦点转移 / 计划切镜序列 / 蒙太奇]
- 镜头路径：[起点、运动、终点；避免与覆盖模式矛盾]
- 动作状态变化：[本镜头唯一变化]
- 终帧状态：[最后 0.5 秒停在哪里，如何剪出]
- 硬限制：[不要推进到下一剧情；用正向目标约束安全边界]

——以下描述动态变化和声音——

0-1.5 秒：[镜头位置/运动：建立画面和动作起点；画面动作：主体做什么；表演因果：触发事件和第一反应]
     [音效：环境音/效果音]
1.5-4 秒：[镜头位置/运动：沿 Shot State Contract 的镜头路径推进；画面动作：主要动作，最多 1-2 个动作 beat；表演因果：表情、身体、语气如何变化]
     [对白：角色台词内容]
4-5 秒：[镜头位置/运动：停在终帧要求的景别/角度；画面动作：动作落稳、反应或结尾可剪点；表演因果：停在具体眼神/呼吸/嘴角/姿态或台词语气]
     [对白：台词] [音效：效果音]
     [配乐：音乐描述]

镜头语言一致性检查：[只总结以上时间轴已经使用的镜头路径，不新增切镜或角度]
结尾：动作自然收住，明确最终姿态、道具状态和可剪辑点。
不变项：[身份/服装/发型/道具/场景/光线方向]
```
...
- [ ] 情绪曲线是否合理递进？
- [ ] 反转/笑点/泪点是否有足够铺垫？
- [ ] 每个重要情绪是否有“发生原因 + 表情细节 + 肢体动作 + 说话语气”？
- [ ] 表情、身体动作、台词语气是否互相一致，而不是只写一个抽象情绪词？
...
- 场景跳变 → 添加过渡镜头或统一场景描述
- 情绪假/跳变 → 给情绪补发生原因，再拆成脸部细节、肢体动作和说话语气
```
