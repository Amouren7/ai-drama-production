...
0-1s：[建立画面和动作起点，保留自然起步；写清谁在何处、看向哪里、道具在哪里]
1-4s：[主要动作，1-2 个动作 beat；每个 beat 都必须是可见动作]
4-5s：[动作落稳或留出反应，明确最终姿态/道具状态/视线方向，给剪辑留 0.5s 余量]
[对白：如有，放在对应时间点]
[音效：环境音、动作音，放在对应时间点]
[配乐：如有，放在对应时间点]
镜头语言：[必须匹配镜头覆盖模式；如果需要手部/眼睛/道具特写，优先拆成 `VIDEO_INSERT`，不要在连续单镜头里要求快速交替特写]
