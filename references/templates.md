...
### 10.2 分镜与视频提示词
| 镜头 | 时长 | 风险 | 视角 | 景别 | 引用视觉素材 | 开始状态 | 结束状态 | 动作节拍 | 表情/心理 | 前后镜关系 | 运镜 | 声音/对白处理 | 提示词ID |
|---|---:|---|---|---|---|---|---|---|---|---|---|---|---|

### 10.3 镜头连接合同 Join Contract
| From | To | 前镜结束状态 | 后镜开始状态 | 状态差异 | 风险等级 | 允许硬切? | 必须桥接? | 桥接提示词ID | 首尾帧适用性 | 保险素材 | 声音桥 | 失败备用剪法 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
...
#### `IMAGE_START_[镜头号]`
```prompt
用途：生成本镜头第一帧。参考：[列出 @图片/@视频 及用途]。9:16 vertical。
画面：[角色/场景/道具的静态构图，明确姿态、屏幕位置、视线、光源、情绪]。
要求：与上一镜头结束状态匹配；无字幕、无水印、无多余人物；保持角色锁和场景锁。
```

#### `IMAGE_END_[镜头号]`
```prompt
用途：生成本镜头最后一帧。参考：[列出 @图片/@视频 及用途]。9:16 vertical。
画面：[动作完成后的状态，明确姿态、屏幕位置、道具状态、视线、光源、情绪]。
要求：能自然接到下一镜头开始状态；无字幕、无水印；保持角色锁和场景锁。
```

#### `IMAGE_MID_[镜头号]`（仅高风险动作需要）
```prompt
用途：生成中间关键帧，锁定动作因果。[例如：抱起、放下、插线、开门、转身、跌落中段]。
画面：[中间状态]。
要求：动作清楚、身体关系合理、道具状态明确。
```

#### `VIDEO_MAIN_[镜头号]`
```prompt
使用参考：[start frame / end frame / middle frame / 角色 / 场景 / 道具]。
时长：[5s/10s]，比例：9:16。
任务：从开始帧自然运动到结束帧，只完成本镜头动作。
0-1s：[建立动作，保留自然起步]
1-4s：[主要动作]
4-5s：[落稳，给剪辑留 0.5s 余量]
[对白：如有]
[音效：环境音、动作音]
[配乐：如有]
